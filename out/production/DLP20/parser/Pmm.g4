...
idList returns [List<String> ast = new ArrayList<String>()] :
			...
			(',' id2=ID {
				if($ast.contains($id2.text)){
                		new ErrorType(new Variable($start.getLine(), $start.getCharPositionInLine() + 1, $id2.text),
                		"Nombre de variable duplicada en linea: " + $start.getLine() + " columna: " +  $start.getCharPositionInLine() + 1);
                }else
                {
                		$ast.add($id2.text);
                }})*
...
defVariableCampos returns [List<FieldRecord> ast = new ArrayList<FieldRecord>() ]: (d=defRecord
			{
            	for (FieldRecord fr: $d.ast) {
            		if ($ast.contains(fr)) {
            			new ErrorType(new Variable($start.getLine(), $start.getCharPositionInLine() + 1, ""),
            			"Campo de struct duplicado en linea:" + $start.getLine() + "y columna: " + $start.getCharPositionInLine()+1);
            		}
            		else {
            			$ast.add(fr);
            		}
